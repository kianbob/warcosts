  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-stone-900 text-white rounded-xl p-8 mb-8">
        ...
        <p className="text-stone-400 mt-2">{country.years} · {fmt(country.totalStrikes)} confirmed strikes</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-8">
        <div className="bg-stone-800 rounded-lg p-4 text-center border border-stone-700">
          <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmt(country.totalStrikes)}</p>
          <p className="text-xs text-stone-400">Total Strikes</p>
        </div>
        <div className="bg-stone-800 rounded-lg p-4 text-center border border-stone-700">
          <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmt(country.minKilled)}–{fmt(country.maxKilled)}</p>
          <p className="text-xs text-stone-400">Total Killed</p>
        </div>
        <div className="bg-stone-800 rounded-lg p-4 text-center border border-stone-700">
          <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmt(country.minCivilians)}–{fmt(country.maxCivilians)}</p>
          <p className="text-xs text-stone-400">Civilians Killed</p>
        </div>
        <div className="bg-stone-800 rounded-lg p-4 text-center border border-stone-700">
          <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmt(country.minChildren)}–{fmt(country.maxChildren)}</p>
      ...
      <div className="space-y-3 mb-10">
        {country.byPresident.map(p => {
          ...
          return (
            <div key={p.president} className="bg-stone-800 border border-stone-700 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-white">{p.president}</span>
                <span className="text-red-400 font-bold">{fmt(p.strikes)} strikes ({pct}%)</span>
              </div>
              <div className="w-full bg-stone-700 rounded-full h-2 mb-1">
                <div className="bg-red-600 h-2 rounded-full" style={{ width: `${pct}%` }} />
              </div>
              <p className="text-sm text-stone-400">{fmt(p.minKilled)}–{fmt(p.maxKilled)} killed</p>
            </div>
          )
        })}
      </div>
      ...
      {/* Year-by-Year */}
      <h2 className="font-[family-name:var(--font-heading)] text-2xl font-bold text-white mb-4">📊 Year-by-Year Breakdown</h2>
      <div className="bg-stone-800 border border-stone-700 rounded-lg p-5 mb-10">
        <div className="space-y-2">
          {country.byYear.map(y => (
            <div key={y.year} className="flex items-center gap-3">
              <span className="text-stone-400 text-sm w-12">{y.year}</span>
              ...
              <span className="text-stone-300 text-sm w-10 text-right">{y.strikes}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-red-950 border border-red-900 rounded-lg p-6 mb-8">
        <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-red-400 mb-3">⚠️ Civilian Casualties</h2>
      ...
      {country.notes && (
        <div className="bg-stone-800 border border-stone-700 rounded-lg p-6 mb-8">
          <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-white mb-3">📝 Notes</h2>
          <p className="text-stone-300 leading-relaxed">{country.notes}</p>
        </div>
      )}

      {country.controversies && (
        <div className="bg-stone-800 border border-stone-700 rounded-lg p-6 mb-8">
          <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-white mb-3">🔥 Controversies</h2>
          <p className="text-stone-300 leading-relaxed">{country.controversies}</p>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        {data.countries.filter(c => c.slug !== slug).map(c => (
          <Link key={c.slug} href={`/drone-strikes/${c.slug}`} className="bg-stone-800 border border-stone-700 rounded-lg px-4 py-2 text-stone-300 hover:bg-stone-700 hover:text-white transition text-sm">
      ...
      <div className="text-center">
        <Link href="/drone-strikes" className="text-red-400 hover:text-red-300 text-sm">← Back to All Drone Strike Data</Link>
      </div>
