...
export default async function CountryBasesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8">
      <Breadcrumbs items={[{ label: 'Bases', href: '/bases' }, { label: 'By Country', href: '/bases/countries' }, { label: c.country }]} />
      ...
      <p className="text-stone-500 text-lg mb-6">{fmt(c.total)} total installations</p>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {c.bases > 0 && <div className="bg-white border border-stone-200 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-red-700">{fmt(c.bases)}</div><div className="text-xs text-stone-400">Major Bases</div></div>}
        {c.lilyPads > 0 && <div className="bg-white border border-stone-200 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-amber-600">{fmt(c.lilyPads)}</div><div className="text-xs text-stone-400">Lily Pads</div></div>}
        {c.usFunded > 0 && <div className="bg-white border border-stone-200 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-blue-400">{fmt(c.usFunded)}</div><div className="text-xs text-stone-400">US-Funded</div></div>}
        {c.domestic > 0 && <div className="bg-white border border-stone-200 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-green-400">{fmt(c.domestic)}</div><div className="text-xs text-stone-400">Domestic</div></div>}
        <div className="bg-white border border-stone-200 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-stone-900">{fmt(c.total)}</div><div className="text-xs text-stone-400">Total</div></div>
      </div>
      ...
      {[{ title: 'Major Bases', items: bases, color: 'red' }, { title: 'Lily Pads & Small Outposts', items: lilyPads, color: 'yellow' }, { title: 'US-Funded Host Nation Bases', items: usFunded, color: 'blue' }, { title: 'Domestic Installations', items: domestic, color: 'green' }].map(section => section.items.length > 0 && (
        <div key={section.title} className="mb-8">
          <h2 className="text-xl font-bold text-stone-900 mb-3">{section.title} ({section.items.length})</h2>
