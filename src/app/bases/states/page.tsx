...
export default function BaseStatesPage() {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      ...
      <p className="text-stone-500 text-lg mb-6">{fmt(states.reduce((s, x) => s + x.total, 0))} installations across {states.length} states &amp; territories</p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
        {states.map((s, i) => (
          <Link key={s.slug} href={`/bases/states/${s.code?.toLowerCase() || s.slug}`} className="bg-white border border-stone-200 hover:bg-stone-50 border border-stone-200 rounded-lg p-4 transition-colors">
            <div className="flex justify-between items-start">
              <div>
                <span className="text-stone-500 text-xs mr-2">#{i + 1}</span>
                <span className="text-stone-900 font-medium">{s.state}</span>
              </div>
              <span className="text-red-700 font-bold">{s.total}</span>
            </div>
