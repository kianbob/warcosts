  ...
  return (
    <div className="min-h-screen bg-stone-900 text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="text-stone-400 mb-8 max-w-2xl">Enter your annual income to see how much of your federal taxes fund the military and foreign aid — broken down by country.</p>

        {/* Input */}
        <div className="bg-stone-800 rounded-xl border border-stone-700 p-6 mb-8">
          <label className="text-stone-400 text-sm block mb-2">Annual Income (before tax)</label>
          <div className="flex gap-3">
            <div className="relative flex-1">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-stone-400">$</span>
              <input type="number" value={income} onChange={e => setIncome(Number(e.target.value))}
                className="w-full bg-stone-700 border border-stone-600 rounded-lg pl-8 pr-4 py-3 text-white text-lg font-mono" />
            </div>
          ...
          <div className="flex gap-2 mt-3">
            {[50000, 75000, 100000, 150000, 250000].map(v => (
              <button key={v} onClick={() => { setIncome(v); setSubmitted(true) }}
                className="px-3 py-1 text-xs bg-stone-700 hover:bg-stone-600 rounded-lg text-stone-300 transition">
                {fmtMoney(v)}
              </button>
            ))}
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-stone-800 rounded-xl border border-stone-700 p-6">
                <p className="text-2xl font-bold text-stone-300 font-[family-name:var(--font-heading)]">{fmtMoney(tax)}</p>
                <p className="text-xs text-stone-400">Federal Income Tax</p>
              </div>
              <div className="bg-stone-800 rounded-xl border border-stone-700 p-6">
                <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmtMoney(militaryShare)}</p>
                <p className="text-xs text-stone-400">→ Military Spending</p>
              </div>
              <div className="bg-stone-800 rounded-xl border border-stone-700 p-6">
                <p className="text-2xl font-bold text-red-400 font-[family-name:var(--font-heading)]">{fmtMoney(aidShare)}</p>
                <p className="text-xs text-stone-400">→ Foreign Aid</p>
              </div>
              <div className="bg-stone-800 rounded-xl border border-stone-700 p-6">
                <p className="text-2xl font-bold text-stone-300 font-[family-name:var(--font-heading)]">{fmtMoney(tax - militaryShare - aidShare)}</p>
                <p className="text-xs text-stone-400">→ Everything Else</p>
              </div>
            </div>
            ...
            <div className="bg-red-950/30 rounded-xl border border-red-900/50 p-6 mb-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="text-center">
                  <p className="text-3xl font-bold text-red-400">{tomahawks.toFixed(1)}</p>
                  <p className="text-stone-400 text-sm">Tomahawk Cruise Missiles</p>
                  <p className="text-stone-500 text-xs">($2M each)</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl font-bold text-red-400">{teachers.toFixed(1)}</p>
                  <p className="text-stone-400 text-sm">Teacher Salaries</p>
            ...
            {/* Aid by Country */}
            <div className="bg-stone-800 rounded-xl border border-stone-700 p-6">
              <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-white mb-4">Your Foreign Aid by Country</h2>
                  ...
                  <div key={c.country} className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-red-400 font-mono">{fmtMoney(c.yourShare)}</span>
                      </div>
