  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <nav className="text-sm text-stone-400 mb-4">
        ...
        <span className="mx-2">›</span>
        <span className="text-white">Budget Simulator</span>
      </nav>
      ...
      <p className="text-stone-300 text-lg mb-2 max-w-3xl">
      ...
      <div className={`rounded-lg border p-4 mb-8 ${
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <span className="text-white font-bold text-xl ml-2">{fmtB(total)}</span>
            <span className="text-stone-500 ml-2">/ {fmtB(TOTAL_BUDGET)} target</span>
          </div>
          <div className="flex gap-2">
            <button onClick={autoBalance} className="text-sm px-3 py-1 bg-stone-800 border border-stone-700 rounded hover:bg-stone-700 text-stone-300 transition">
              Auto-balance
            </button>
            <button onClick={resetAll} className="text-sm px-3 py-1 bg-stone-800 border border-stone-700 rounded hover:bg-stone-700 text-stone-300 transition">
              Reset
            </button>
          </div>
        </div>
            ...
            <div key={cat.id}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-white font-mono font-bold w-16 text-right">{fmtB(val)}</span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <input
                  ...
                  style={{ background: `linear-gradient(to right, var(--tw-gradient-from, #ef4444) ${(val / 2500) * 100}%, #292524 ${(val / 2500) * 100}%)` }}
                />
                <input
                  type="number"
                  min={0}
                  ...
                  value={val}
                  onChange={e => handleChange(cat.id, Number(e.target.value))}
                  className="w-20 bg-stone-800 border border-stone-700 rounded px-2 py-1 text-white text-sm text-right"
                />
              </div>
              {/* Default marker */}
          ...
          return (
            <div key={cat.id}>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-white w-14">Yours</span>
                  <div className="flex-1 h-4 bg-stone-800 rounded-full overflow-hidden">
                    <div className={`h-full ${cat.barColor} rounded-full`}
                      style={{ width: `${(val / maxVal) * 100}%` }} />
                  </div>
                  <span className="text-xs text-white w-14 text-right">{fmtB(val)}</span>
                </div>
              </div>
            </div>
          )
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-10">
        {militaryDelta !== 0 && (
          <div className={`rounded-lg border p-5 ${militaryDelta < 0 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
            <h3 className="font-bold text-white mb-2">🎖️ Military {militaryDelta < 0 ? 'Cut' : 'Increase'}</h3>
        ...
        {educationDelta > 0 && (
          <div className="rounded-lg border p-5 bg-amber-500/10 border-amber-500/30">
            <h3 className="font-bold text-white mb-2">📚 Education Boost</h3>
            <p className="text-stone-300 text-sm">
        ...
        {healthcareDelta > 0 && (
          <div className="rounded-lg border p-5 bg-emerald-500/10 border-emerald-500/30">
            <h3 className="font-bold text-white mb-2">🏥 Healthcare Expansion</h3>
            <p className="text-stone-300 text-sm">
        ...
        {infraDelta > 0 && (
          <div className="rounded-lg border p-5 bg-purple-500/10 border-purple-500/30">
            <h3 className="font-bold text-white mb-2">🏗️ Infrastructure Investment</h3>
            <p className="text-stone-300 text-sm">
        ...
        {aidDelta > 0 && (
          <div className="rounded-lg border p-5 bg-teal-500/10 border-teal-500/30">
            <h3 className="font-bold text-white mb-2">🌍 Foreign Aid Increase</h3>
            <p className="text-stone-300 text-sm">
        ...
        {militaryDelta < 0 && educationDelta > 0 && (
          <div className="rounded-lg border p-5 bg-blue-500/10 border-blue-500/30 md:col-span-2">
            <h3 className="font-bold text-white mb-2">💡 The Trade-Off</h3>
            <p className="text-stone-300 text-sm">
              Moving {fmtB(Math.abs(militaryDelta))} from defense to education would hire{' '}
              <strong className="text-white">{(Math.abs(militaryDelta) * IMPACT_PER_BILLION.education.teachers).toLocaleString()} teachers</strong> and build{' '}
              <strong className="text-white">{(Math.abs(militaryDelta) * IMPACT_PER_BILLION.education.schools).toLocaleString()} schools</strong>.
      ...
      {/* Context */}
      <div className="bg-stone-800/50 rounded-xl border border-stone-700 p-6 mb-8">
        <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold mb-3">For Context</h3>
        <ul className="space-y-2 text-stone-300 text-sm">
          <li>• The US spends more on its military than the next <strong className="text-white">10 countries combined</strong></li>
          <li>• Universal pre-K for all American children would cost ~<strong className="text-white">$30B/year</strong></li>
          <li>• Eliminating all student loan debt: ~<strong className="text-white">$1.7T one-time</strong></li>
          <li>• Clean water for every human on Earth: ~<strong className="text-white">$150B total</strong></li>
          <li>• The F-35 program alone costs <strong className="text-white">$1.7T lifetime</strong> — more than most categories above</li>
        </ul>
      </div>
