    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 my-8">
        <div className="bg-white border border-stone-200 rounded-lg p-4 text-center shadow-sm">
          <div className="text-2xl font-bold text-red-700">{sanctions.length}</div>
          <div className="text-stone-500 text-sm">Sanctions Programs</div>
        </div>
        <div className="bg-white border border-stone-200 rounded-lg p-4 text-center shadow-sm">
          <div className="text-2xl font-bold text-red-700">{fmtMoney(totalCost * 1_000_000)}</div>
          <div className="text-stone-500 text-sm">Est. Economic Impact</div>
        </div>
        <div className="bg-white border border-stone-200 rounded-lg p-4 text-center shadow-sm">
          <div className="text-2xl font-bold text-red-700">{Math.max(...sanctions.map(s => new Date().getFullYear() - s.startYear))}+</div>
          <div className="text-stone-500 text-sm">Longest Active (Years)</div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-10">
        {sanctions.map((s) => (
          <Link key={s.slug} href={`/sanctions/${s.slug}`} className="group block">
            <div className="bg-white border border-stone-200 rounded-lg p-5 hover:border-red-300 hover:shadow-md transition-all h-full">
              <div className="flex items-center justify-between mb-2">
                <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold text-stone-900 group-hover:text-red-700 transition-colors">{s.name}</h2>
                <span className="text-xs bg-red-50 text-red-700 border border-red-200 px-2 py-1 rounded font-semibold">{s.status}</span>
              </div>
              ...
              <p className="text-stone-600 text-sm line-clamp-3">{s.description}</p>
            </div>
          </Link>
        ))}
      </div>
